...
def test_main_generates_input_checkout_from_repo(
    ...
    monkeypatch.setattr(
        ...
        lambda repo, *, create_commit, clone_token, vars_from, output_subdir: (
            calls.append((repo, create_commit, clone_token, vars_from, output_subdir))
            or generated_dir
        ),
    )
    monkeypatch.setattr(
    ...
    assert result.exit_code == 0
    assert calls == [
        ("https://github.com/nresare/manifests.git", False, None, None, None)
    ]
...
def test_main_forwards_target_clone_token(
    ...
    assert result.exit_code == 0
    assert calls == [
        ("https://github.com/nresare/manifests.git", False, "clone-token", None, None)
    ]
...
def test_main_forwards_target_subdir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    generated_dir = tmp_path / "output"
    generated_dir.mkdir()
    ...
    assert result.exit_code == 0
    assert calls == [
        ("https://github.com/nresare/manifests.git", False, None, None, "platform-dev")
    ]
